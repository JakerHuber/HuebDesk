...
public class FifthAdminPresenter {

	// Values for form
	private Dialog<String> createNoteDialog;

	@FXML
	private View fifthAdmin;
	...
	public void initialize() {
		...
		fifthAdmin.setShowTransitionFactory(v -> new FadeInRightTransition(v));
		...
		fab.showOn(fifthAdmin);
		

		fifthAdmin.showingProperty().addListener((obs, oldValue, newValue) -> {
			if (newValue) {
		...
		fifthAdmin.setOnShown(e -> {
			// Create note list
			createList();
		});
		
		fifthAdmin.setOnHidden(g -> {
	...
	void createNote(String emailcc, String emailbcc, String notebody) {
		...
		if (!notebody.equals("")) {
			...
			int note_id = GluonApplication.currentAdmin.getDbNoteNextAutoincrement();

			// Create database note
			GluonApplication.currentAdmin.createDbNote(note_id, new Timestamp(System.currentTimeMillis()),
					new Timestamp(System.currentTimeMillis()), emailcc, emailbcc, notebody,
					GluonApplication.currentAdmin.getEmployee_id());
			...
			GluonApplication.currentAdmin.createDbNoteTicket(GluonApplication.currentAdminTicket.getTicket_id(), note_id);
	...
	void createList() {
		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				// Load data
				ObservableList<Notes> allnotes = GluonApplication.currentAdmin.getAdminTicketNotes(GluonApplication.currentAdminTicket.getTicket_id());
				if(!allnotes.isEmpty()){
					...
					public void updateItem(Notes item, boolean empty) {
						...
						try {
							if (item != null && !empty) {
								...
								final String authorname = GluonApplication.currentAdmin.getAdminAuthorName(authorIdValue);
								...
								// Add details
								if (item.getNauthor_employee_id() == GluonApplication.currentAdmin.getEmployee_id()) {
