...
public class SecondaryPresenter {
	...
	public void initialize() {
		secondary.setShowTransitionFactory(v -> new FadeInRightTransition(v));
		...
		secondary.showingProperty().addListener((obs, oldValue, newValue) -> {
			if (newValue) {
				...
				appBar.setTitleText("View Current Tickets");
			}
	...
	void createList() {
		...
		Platform.runLater(new Runnable() {
			...
			public void run() {
				...
				// Load data
				ObservableList<Tickets> alltickets = GluonApplication.currentUser.getUsersAllTickets();
				if(!alltickets.isEmpty()){
					charmlist1.setItems(alltickets);
				}
				else {
					charmlist1.setPlaceholder(new Label("This user has no tickets"));
				}
				...
				charmlist1.selectedItemProperty().addListener((observable, olditem, newitem) -> {
						GluonApplication.currentUserTicket = newitem;
